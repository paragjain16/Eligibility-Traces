...
//            ((QLearningAgent)(this.agent)).setRate(((QLearningAgent)(this.agent)).getRate()/(episode+1));
//            ((QLearningAgent)(this.agent)).setEpsilon(((QLearningAgent)(this.agent)).getEpsilon()/(episode+1));
            ...
            for (int step = 0; step < this.steps; step++) {
                ...
                int nextAction = this.agent.chooseAction(newState);
                int bestAction = ((QLearningAgent)agent).bestAction(newState);
                boolean isGreedy = true;
                if(bestAction != nextAction)
                    isGreedy = false;

                this.agent.updatePolicyET(reward, action, bestAction, currState, newState, isGreedy);
